...
class LightingRepository(context : Context){
    ...
    fun saveProductesData(productesData: List<Products>) {
        ...
        sharedPreferences.edit().putString("lightingData", json).apply()
    }

    fun getCachedproductesData(): List<Products> {
        val json = sharedPreferences.getString("lightingData", null)
        val gson = Gson()
    ...
    fun clearCachedProductesData() {
        sharedPreferences.edit().remove("lightingData").apply()
    }
